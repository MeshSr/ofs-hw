...
public:
	...
	*/
	...
	/*!
		...
		\return A pointer to a _nbExtractedFieldsDescriptorVector if no error occurred, NULL otherwise.
		The _nbExtractedFieldsDescriptorVector returned is empty ( _nbExtractedFieldsDescriptorVector->n=0 )
		in case the filtering string (entered by the user) does not contain any field to be extracted.
	*/
	virtual _nbExtractedFieldsDescriptorVector* GetFields()=0;
...
protected:
	/*!
		...
		\note 
		for “allfields” descriptor it creates a new _nbExtractedFieldsDescriptorVector structure and 
